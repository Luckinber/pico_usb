...
from .utils import EP_IN_FLAG, Buffer
...
class MIDIInterface(USBInterface):
    ...
    #
    # To be compliant this also regisers a dummy USB Audio interface, that isn't otherwise
    # used.
    ...
    def send_event(self, cin, midi0, midi1, midi2):
        # Helper function to queue a MIDI Event Packet to send to the host.
        #
    ...
    def on_midi_event(self, cin, midi0, midi1, midi2):
        # Override this function in a subclass in order to react to MIDI data
        pass
...
class MIDIUSB(MIDIInterface):
    # Simple USB MIDI base class which adds convenience wrappers for MIDI note
    # on, note off, and control change events.
